...
uint8_t receiver_address[] = {0x10, 0x91, 0xA8, 0x32, 0x6B, 0xA8}; // Mac address of the receiver.
uint8_t local_address[] = {0x10, 0x91, 0xA8, 0x33, 0xD3, 0x9C}; 
...
unsigned long t_send_espnow = 0;
bool motion_reading = true;
...
void loop() {
  ...
  // Send motion data to the receiver
  if(millis() >= t_send_espnow + 50){
    t_send_espnow = millis();
    leds[0] = CRGB::Orange;
    leds[1] = CRGB::Orange;
    FastLED.show();

    data.motion_x = lis.readFloatAccelX();
    data.motion_y = lis.readFloatAccelY();
    data.motion_z = lis.readFloatAccelZ();
    esp_now_send(receiver_address, (uint8_t *) &data, sizeof(data));
  }
}
